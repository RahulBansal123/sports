...
def send_otp(email , otp,username):
    ...
    emailSend = EmailMessage(sub, msg, emailFrom,to= [email,])
    emailSend.send()
    print("gg4")    
...
def getRegister(request):
    ...
    if request.method == 'POST':
        ...
        siteUser = SiteUserModel.objects.create(user = user_obj, name=name,email=email,phoneNumber=phone, passwordHash = str(hashed, 'UTF-8'),salt = str(salt, 'UTF-8'))
